...
static void bcm2835_spi_dma_tx_done(void *data)
{
	...
	if (
      ( 
        { 
			typeof(&bs->rx_dma_active) __ai_ptr = (&bs->rx_dma_active); 
        //   instrument_atomic_write(__ai_ptr, sizeof(*__ai_ptr)); 
          (
              { 
                __typeof__(*(__ai_ptr)) __ret; 
                __ret = (__typeof__(*(__ai_ptr))) __cmpxchg_mb((__ai_ptr), (unsigned long)(true), (unsigned long)(false), sizeof(*(__ai_ptr))); 
                __ret; 
              }
          ); 
        }
